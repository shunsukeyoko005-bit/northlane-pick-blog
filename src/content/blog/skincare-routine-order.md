---
title: "The Correct Order to Apply Skincare (Morning & Night)"
slug: "skincare-routine-order"
description: "Applying products in the right order helps each one actually work. Here's the simple morning and night order — from cleanser to SPF — plus easy picks on Amazon."
cluster: "skincare-routine"
status: "sample-draft"
created: "2026-06-30"
affiliate_tag: "northlanepick-20"
cover_image: "/images/covers/skincare-routine-order.jpg"
---

*As an Amazon Associate I earn from qualifying purchases. Some links below are affiliate links, which means I may earn a commission at no extra cost to you.*

# The Correct Order to Apply Skincare (Morning & Night)

If your serums feel like they "do nothing," the problem might not be the products — it might be the **order** you put them on. Applying skincare in the wrong sequence can block ingredients from absorbing and leave your skin unprotected. Here's the simple rule that fixes it.

## The one rule that makes it easy

**Apply from thinnest to thickest.** Watery, lightweight products go first so they can absorb into clean skin. Heavier creams and SPF go last to seal everything in.

## Morning routine (AM)

1. **Cleanser** — Start with a gentle cleanser to remove overnight oil. A simple foaming cleanser like the [CeraVe Foaming Facial Cleanser](https://www.amazon.com/dp/B00TTD9BRC?tag=northlanepick-20) works for normal-to-oily skin.
2. **Serum (optional)** — A lightweight active such as [The Ordinary Niacinamide 10% + Zinc 1%](https://www.amazon.com/dp/B01LXAZL30?tag=northlanepick-20) goes on bare skin so it absorbs first.
3. **Moisturizer** — Lock in hydration with a cream like the [CeraVe Moisturizing Cream](https://www.amazon.com/dp/B00TTD9CR8?tag=northlanepick-20).
4. **SPF (always last)** — Sunscreen is the final step every single morning. A lightweight fluid like the [La Roche-Posay Anthelios Ultra-Light Fluid SPF 60](https://www.amazon.com/dp/B002CML1XE?tag=northlanepick-20) sits well under makeup.

> **Quick tip:** SPF always goes on *after* moisturizer, never before. If you wear makeup, that goes on top of sunscreen.

## Night routine (PM)

1. **Cleanser** — Remove makeup, SPF, and the day's grime.
2. **Serum / treatment** — This is the time for actives (like retinol, if your skin tolerates it).
3. **Moisturizer** — A slightly richer cream at night helps repair while you sleep.

You skip SPF at night — that's the only difference from your morning routine.

## Common mistakes to avoid

- **Moisturizer before serum** — the cream blocks the serum from absorbing.
- **Skipping SPF on cloudy days** — up to 80% of UV still gets through clouds.
- **Layering too fast** — give each step 30–60 seconds to absorb.

## A simple starter set

If you're building a routine from scratch, four products cover it: a cleanser, one serum, a moisturizer, and an SPF. Keep them lined up on your counter in order so you never skip a step.

- Cleanser: [CeraVe Foaming Facial Cleanser](https://www.amazon.com/dp/B00TTD9BRC?tag=northlanepick-20)
- Serum: [The Ordinary Niacinamide 10% + Zinc 1%](https://www.amazon.com/dp/B01LXAZL30?tag=northlanepick-20)
- Moisturizer: [CeraVe Moisturizing Cream](https://www.amazon.com/dp/B00TTD9CR8?tag=northlanepick-20)
- SPF: [La Roche-Posay Anthelios Fluid SPF 60](https://www.amazon.com/dp/B002CML1XE?tag=northlanepick-20)

*Check current prices and reviews on Amazon — they update often.*

---

*As an Amazon Associate I earn from qualifying purchases.*
